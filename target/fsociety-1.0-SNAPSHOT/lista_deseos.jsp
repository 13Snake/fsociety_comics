<%-- 
    ...
    Created on : 14 dic 2022, 00:00:15
    Author     : ksio
--%>
...
<%
    if (request.getAttribute("mensaje") != null) {
        ...
        out.println("<script>window.location='lista_deseos.jsp'</script>");
    }

    String bt_nums = request.getParameter("pos");
    int posicion;
    int bandera = 0;
    ArrayList<productos> res_bus = new ArrayList();
    float nums_lista = 1;
    String busqueda = "";
    if(bt_nums == null){
        posicion = 1;
    ...
    if (session.getAttribute("idusuario") == null) {
        out.println("<script>alert('Inicia sesión o registrate para poder ver tu lista de deseos'); </script>");
        out.println("<script>window.location='login.jsp?opt=3'</script>");
    }else{
        busqueda = session.getAttribute("idusuario").toString();
        res_bus = conection_db.lista_deseos(busqueda);
        int cantidad = res_bus.size();
        nums_lista = cantidad/8;
        ...
        <div class="containter-fuid py-4">
            <%
                if(res_bus.isEmpty()){
                    ...
                    out.println("<h5 class='card-title'>¡Tu lista esta vacia!</h5>");
                    out.println("<p class='card-text'>No esperes más y corre a obtener los productos que más te gustan</p>");
                    ...
                    for(int x = inicio; x<fin; x++){
                        ...
                        out.println("<a type='button' href='addwish?opt=2&idprod="+prod.getIdproducto()+"'>");
                        out.println("<span class='badge text-bg-light'>");
                        out.println("<img src='assets/icons/hlleno.png' width='30' height='30' class='img-fluid txt_nav d-inline-block'>");
                        out.println("</span>");
                        out.println("</a>");
                        out.println("</div>");
                        out.println("</div>");
                                    ...
                                    if (nums_lista > 1 && !res_bus.isEmpty()) {
                                        for (int x = 1; x <= nums_lista; x++) {
                                            if (posicion == x) {
                                                ...
                                                out.println("<li class='page-item'><a class='page-link' href='lista_deseos.jsp?pos=" + x + "'>" + x + "</a></li>");
                                                out.println("</li>");
                                            }
                                        }
                                    }
